              ...
              <div className="mb-8">
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-5">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <p className="text-base font-semibold text-slate-900">{medicine.shop?.name || "Premium Pharmacy"}</p>
                    {locating ? (
                      <span className="shrink-0 bg-slate-100 text-slate-400 text-[10px] font-bold px-2.5 py-1 rounded-lg border border-slate-200 whitespace-nowrap animate-pulse">
                        Locating...
                      </span>
                    ) : medicine.shop?.distance !== null && medicine.shop?.distance !== undefined ? (
                      <span className="shrink-0 bg-emerald-50 text-emerald-700 text-xs font-bold px-2.5 py-1 rounded-lg border border-emerald-100 whitespace-nowrap">
                        📍 {medicine.shop?.distance} km away
                      </span>
                    ) : locationGranted === false ? (
                      <button
                        onClick={getLocation}
                        className="shrink-0 bg-amber-50 text-amber-600 text-[10px] font-semibold px-2.5 py-1 rounded-lg border border-amber-100 whitespace-nowrap hover:bg-amber-100 transition-colors"
                      >
                        Enable location
                      </button>
                    ) : (
                      <span className="shrink-0 bg-slate-100 text-slate-400 text-[10px] font-semibold px-2.5 py-1 rounded-lg border border-slate-200 whitespace-nowrap">
                        — km
                      </span>
                    )}
                  </div>
