...
import { applyDoctor, getApplicationStatus } from "@/services/apis/doctor.api";
...
export default function DoctorApplyPage() {
  ...
  const handleGetLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://medifindapiii.duckdns.org/api"}/locations/reverse?lat=${lat}&lng=${lng}`);
  ...
  const validate = (currentStep: number | "all") => {
    let newErrors: any = {};
    ...
    if (currentStep === 2 || currentStep === "all") {
      ...
      if (!formData.qualification.university) newErrors.university = "University is required";
      if (!formData.specialization) newErrors.specialization = "Specialization is required";
      if (!formData.registrationNumber) newErrors.regNo = "Registration number is required";
      if (!formData.medicalCouncil) newErrors.medicalCouncil = "Medical council is required";
    }
    setErrors(newErrors);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error("Doctor apply error:", err.response?.data || err);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          className={`w-full bg-white border ${errors.university ? 'border-red-500 ring-4 ring-red-50' : 'border-slate-300 focus:border-[#0a4d33]'} rounded-2xl px-6 py-4 outline-none transition-all text-black font-semibold placeholder:text-slate-300 placeholder:font-normal`}
                        />
                        {errors.university && <p className="text-[10px] text-red-500 font-bold ml-2">{errors.university}</p>}
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          className={`w-full bg-white border ${errors.medicalCouncil ? 'border-red-500 ring-4 ring-red-50' : 'border-slate-300 focus:border-[#0a4d33]'} rounded-2xl px-6 py-4 outline-none transition-all text-black font-semibold placeholder:text-slate-300 placeholder:font-normal`}
                        />
                        {errors.medicalCouncil && <p className="text-[10px] text-red-500 font-bold ml-2">{errors.medicalCouncil}</p>}
                      </div>
