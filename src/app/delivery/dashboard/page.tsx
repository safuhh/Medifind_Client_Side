...
import { getDeliveryBoyDashboard, getAvailableOrders, acceptOrder, pickupOrder, deliverOrder, connectStripe } from "@/services/apis/deliveryboyapi";
...
export default function DeliveryDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (data?.userId) {
      const id = data.userId._id || data.userId;
      socket.emit("join_delivery_boy_room", id);
    }
            ...
            <div key={order._id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-sm text-slate-500">{order.items?.length || 0} items â€¢ ₹{order.totalAmount}</p>
                </div>
                <button
